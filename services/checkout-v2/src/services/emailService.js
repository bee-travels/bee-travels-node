...
*/
  ...
  checkout_object.cart_items.forEach((item) => {
    ...
    lines.push("");
    lines.push(
      `Item${item.cart_item_type} in the amount of:  ${item.currency}${item.cart_item_cost}`
    );
    lines.push("");
  });
  ...
  lines.push("");
  lines.push(
    `Total amount paid was ${checkout_object.currency}${checkout_object.total_amount}`
  );
